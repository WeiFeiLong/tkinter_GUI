...
class FATHER(tkinter.Tk):
    ...
    def set_father_window(self):
        ...
        self.secorde = scrolledtext.ScrolledText(self.init_window_name, width=40, height=6)  # 发送结果
        self.secorde.place(relx=x2[6], rely=y_show, anchor=CENTER)
        self.recorde = scrolledtext.ScrolledText(self.init_window_name, width=40, height=6)  # 接收结果
        ...
        self.wendu_label.place(relx=0.05, rely=0.85, anchor=CENTER)
    ...
    def started(self, ip, port):
        global connect, get_data_31_old
        if connect == 0:
            self.sk = socket.socket()
    ...
    def setLine2(self):
        self.w = Canvas(self.init_window_name, width=2000, height=10)  # 实际线宽由 height=10控制
        self.w.create_line(0, 0, 2000, 0, fill="#000000", width=30, capstyle=ROUND)  # 真实线宽 多余的被Canvas()遮挡了
        self.w.place(relx=0.5, rely=0.80, anchor=CENTER)  # Canvas上中心点位置
    ...
    def recvdata(self):
        global get_data_31, get_data_31_old
        print('开始接收数据。。。')
        ...
        if connect:
            ...
            try:
                ...
                for y in range(len(list_readstr) // 7):  # 把此次收到的数据都解析
                    ...
                    if list_3_shi:  # 有3个温度
                        if list_y[0] == 90:  # 匹配Z, 此路有平均温度
                            self.avg_num_0.set(round(reduce(lambda q, r: q + r, list_3_shi) / 3, 1))  # Z，A三路均值
                ...
                avg_num_sum=round(reduce(lambda q, r: q + r, self.trans_200(get_data_31_old[0])[:30]) / get_data_31_old[1], 1)  # 计算30路平均值
                ...
                print('err_接收', e)
        return get_data_31  # 返回给类 SON 的温度数据 [1*31]，获取到的即为温度，否则值为-200

    def sendAllData(self, s):  # #
        global connect, hot_or_col, get_data_4hex_huan  # 00 24
        if connect:
            ...
            if self.sendAllBuff == '':  # 为空
                ...
                self.sendAllBuff = self.get_h_l(self.int_transform_4hex(round(float(self.sendAllBuff), 1)))  # 23,ffce,(0, 230)
                if int(hot_or_col) == 1:
                    s.send(bytes('#+', 'utf8'))  # 通过网口进行发送
                    s.send(self.sendAllBuff[0].to_bytes(1, byteorder='big'))  # 通过网口进行发送
                    s.send(self.sendAllBuff[1].to_bytes(1, byteorder='big'))  # 通过网口进行发送
    ...
    def send9Data(self, s):  # J
        ...
        if connect:
            ...
            if self.send9Buff == '':  # 为空
                ...
                self.send9Buff = self.get_h_l(self.int_transform_4hex(round(float(self.send9Buff), 1)))  # ff ce, (255 206)
                if int(hot_or_col) == 1:
    ...
    # 输入[87, 0, 215, 0, 221, 0, 225]，输出[21.5, 22.1, 22.5]（已除以10）
    def t6_get_3t(self, arges):
        ans = []
        if len(arges) == 7:
            for i in range(3):
                ans_1 = arges[1 + 2 * i] * 16 ** 2 + arges[2 + 2 * i]
                ans.append(ans_1 / 10)
        return ans
