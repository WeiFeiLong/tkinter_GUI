...
class FATHER(tkinter.Tk):
    ...
    def started(self, ip, port):
        global connect, get_data_31_old, t_set, avg_num_sum, send_data_times, t_get_new, hot_or_col_s, num_times  # 初始化参数
        t_set = -100  # 初始化温度手动设置数值
        avg_num_sum = -200  # 初始化温度,手动设置数值
        t_get_new = 1  # 初始化开始时间
        hot_or_col_s = 2
        # num_times = 0  # 统计接收数据次数
        if connect == 0:
            self.sk = socket.socket()
            ...
            print('连接成功')
            connect = 1
            send_data_times = 0  # 初始化全局温度设置按钮的点击次数
    ...
    # 关闭连接 并在日志框显示
    def closed(self):
        global connect, t_close
        t_close=time.time()  # 刷新关闭时的时间
        if connect == 1:
            self.wendu_data.insert(INSERT, 'waiting...\n')  # 文本展示区 展示
    ...
    def set_hands_temp(self, now_temp, target_temp, t_use):
        global connect, t_send_new, t_set, t_close  # 一般比下面的开始时间小，除非重新点击了设置按钮
        ...
        for i in range(1, t_use + 1):
            if connect:  # 随时可跳出循环
                t_set = round((now_temp + i * t_s), 1)  # 生成随机（0， 1）
                time.sleep(1)
                if 1:
                    self.avg_num.set(t_set)  # 30路平均，值
                if t_send_new > t_begin or t_close>t_begin:
                    break
            else:
                break
        while 1:  # 设置完后，动态显示
            random_t = random.randint(-1, 1) * random.random()  # 动态生成
    ...
    def recvdata(self):
        global get_data_31, get_data_31_old, avg_num_sum, t_set, send_data_times, t_send_new, hot_or_col, hot_or_col, num_times, hot_or_col_s
        print('开始接收数据。。。')
        ...
        if connect:
            ...
            try:
                ...
                t_get_new = time.time()  # 每次循环  更新点击时间
                if t_get_new - t_send_new < 10 and send_data_times == 1:  # 10秒之内,第一次 正常显示
                    self.avg_num.set(avg_num_sum)  # 30路平均，值
                else:  # 10秒之外 变化显示
                    num_times += 1  # 统计接收函数次数
                    self.write_wendu_log(num_times)
                    if num_times == 1:  # 只接收一次
                        if int(hot_or_col_s) == 1:  # 加热
                            if t_set == -100:  # 第一次
                                if avg_num_sum != -200:  # 有平均温度
                                    self.set_hands_temp(avg_num_sum, float(self.sendAllSet.get()), 80)  # 升温50s
                                    self.write_wendu_log('wei1')
                            else:
                                self.set_hands_temp(t_set, float(self.sendAllSet.get()), 80)  # 升温50s
                                self.write_wendu_log('fei1')
                        elif int(hot_or_col_s) == 0:  # 降温
                            if t_set == -100:  # 第一次
                                if avg_num_sum != -200:  # 有平均温度
                                    self.set_hands_temp(avg_num_sum, self.sendAllSet.get(), 80)  # 降温50s
                                    self.write_wendu_log('wei0')
                            else:
                                self.set_hands_temp(t_set, self.sendAllSet.get(), 80)  # 降温50s
                                self.write_wendu_log('fei0')
    ...
    def sendAllData(self, s):  # #
        global connect, hot_or_col, get_data_4hex_huan, t_get_new, t_send_new, t_set, avg_num_sum, send_data_times, hot_or_col_s, num_times  # 00 24
        # avg_num_sum=1
        if connect:
            ...
            if self.sendAllBuff == '':  # 为空
                ...
                if int(hot_or_col) == 1:
                    ...
                    # if t_set == -100:  # 第一次
                    #     if avg_num_sum != -200 and t_get_new-t_send_new>11:  # 有平均温度
                    #         self.set_hands_temp(avg_num_sum, float(self.sendAllSet.get()), 80)  # 升温50s
                    # else:
                    #     self.set_hands_temp(t_set, float(self.sendAllSet.get()), 80)  # 升温50s
                else:
                    ...
                    # else:
                    #     self.set_hands_temp(t_set, self.sendAllSet.get(), 80)  # 降温50s
                # print('shijian:',t_get_new, t_send_new)
